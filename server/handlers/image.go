...
func buildPrompt(stage, generation int) string {
	...
	animalIndex := stage - 1
	if animalIndex < 0 { animalIndex = 0 }
	if animalIndex > 2 { animalIndex = 2 }
...
func generateGardenImage(prompt string, userID string, stage int) ([]byte, error) {
	...
	// リクエストボディ: プロンプトと画像サイズ（1:1 に設定）[cite: 3]
	reqData := map[string]interface{}{
		"prompt": prompt,
		"width":  1024,
		"height": 1024,
	...
	if stage > 1 {
		...
		if dataDir == "" {
			dataDir = "./data/images" //[cite: 3]
		}
		// 前のステージの画像を読み込む[cite: 3]
	...
	// 画像データが大きくなるため、タイムアウトを長めに設定[cite: 3]
	...
	// Cloudflare Workers AI はバイナリ（PNG/JPEG）をそのまま返す[cite: 3]
	imgData, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("レスポンス読み込み失敗: %w", err)
	}
...
// GenerateAndSaveGardenImage は非同期（goroutine）で呼び出す[cite: 3]
// 生成完了後に gardens テーブルの image_url を更新する[cite: 3]
func GenerateAndSaveGardenImage(gardenID, stage, generation int, userID string, updateImageURL func(gardenID int, imageURL string)) {
	go func() {
		prompt := buildPrompt(stage, generation) //[cite: 3]
		...
		imageURL, err := saveToLocalFile(imgData, userID, generation, stage) //[cite: 3]
		if err != nil {
			log.Printf("[画像生成] 保存エラー: %v", err)
			return
		}

		updateImageURL(gardenID, imageURL) //[cite: 3]
